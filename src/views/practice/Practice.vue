  ...
  <div>
    <el-container class="outside">
      ...
      <el-container>
            ...
            <el-card class="box-card-question" v-if="subject.kind === 'singleq'" v-show="questionIndex===questionID">
              ...
              <div slot="header" class="clearfix">
                <span class="ques-head">
                  ...
                  {{questionIndex + 1}}、({{subject.scorce}}分){{subject.question}}
                </span>
              </div>
        ...
        <el-aside>
          <el-card class="box-card-sheet" shadow="always" body-style="align-items:center">
            ...
            <template #header>
              ...
              <div class="tag-group" ref="tags">
                <el-tag type="primary" effect="plain" v-for="index in subjects.length" style="cursor: pointer"
                  @click="goToQues(index)" shadow="hover">
                  {{index}}
                </el-tag>
              </div>
            </template>
            <!-- <el-button class="button" type="text" @click="submitAll">提交</el-button> -->
            <span>答对 : {{ $store.getters.rightNum }} 答错 : {{ $store.getters.errorNum }} 正确率 :
              {{ $store.getters.rightRate }}</span>
          </el-card>
        </el-aside>
      </el-container>
    </el-container>
  ...
  export default {
    ...
    methods: {
      ...
      nextSubject() {
        ...
        this.analysis = false
        // console.log("正确数量" + this.$store.getters.rightNum);
      },
      //点击上一题触发
      prevSubject() {
        //判断是否是第一题
      ...
      getChoice(quesIndex, choiceIndex, subject) {
        ...
        if (correctness === false) {
          this.$refs.tags.children[quesIndex].className = 'el-tag el-tag--danger el-tag--dark'
        } else {
      ...
      submitAll() {
        ...
        if (storeLen < subjectsLen) {
          ...
          this.$alert(
            '<strong> 总共 {{ this.subjects.length }} 道题，答对 {{ this.$store.state.result }} 道题，<br> 得分 {{ this.$store.state.score }}</strong>',
            '答题情况', {
...
<style scoped>
  ...
  input[type="radio"]:checked+span::after {
    background: #409EFF;
    /* background: "@assets/images/correct-256"; */
  }
